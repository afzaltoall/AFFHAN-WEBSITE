...
export interface CardItem {
  ...
  alt?: string;
  linkUrl?: string;
  /** When set, clicking the card calls this instead of navigating. */
  onSelect?: () => void;
  /** Small overline shown at the top of the card (e.g. category). */
  tag?: string;
...
const FAN_POSITIONS = [
  { rot: -18, scale: 0.85, x: -26, y: 3.5, zIndex: 1 },
  { rot: -12, scale: 0.9,  x: -18, y: 1.5, zIndex: 2 },
  { rot: -6,  scale: 0.95, x: -9,  y: 0.4, zIndex: 3 },
...
function getHeightMultiplier(width: number) {
  let idealPx: number;
  if (width < 480) idealPx = 19 * 16;
  else if (width < 640) idealPx = 22 * 16;
      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const inner = (
              <>
                <div className="relative w-full h-full overflow-hidden">
                  <Image src={getCdnUrl(card.imgUrl) as string} fill sizes="(max-width: 768px) 200px, 300px" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
                  {(card.tag || card.title) && (
                    <div className="absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-black/75 via-black/25 to-transparent p-3 pt-8 text-left">
                      ...
                      {card.title && <span className="mt-0.5 block text-xs font-semibold text-white line-clamp-2 leading-snug">{card.title}</span>}
                    </div>
                  )}
                </div>
              </>
            ...
            if (card.onSelect) {
              return (
                <button key={index} type="button" onClick={card.onSelect} className="fan-card block cursor-pointer text-left">{inner}</button>
              );
            }
            return card.linkUrl ? (
