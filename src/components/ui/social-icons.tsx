...
/**
 * Social marks, drawn monochrome in `currentColor`.
 *
 * These used to carry each platform's own brand colour — Instagram's four-stop
 * gradient, LinkedIn #0A66C2, YouTube red, Facebook blue, TikTok and X black.
 * Six competing brand palettes in one nine-pixel row read as clip-art dropped
 * onto the page, and on the contact card's pale teal glass they actively fought
 * the surface they sat on.
 *
 * Inheriting the colour instead means the row takes the site's palette from
 * whatever wraps it, so hover and focus states work by changing one text colour
 * on the link rather than by restyling six different SVGs. The silhouettes are
 * unchanged, which is the part people actually recognise.
 *
 * Every mark is a single path using fill-rule knockout for its counter (the "f",
 * the play triangle, the "in"), because a two-tone icon cannot be recoloured by
 * a parent — the second tone would stay hardcoded.
 *
 * Callers must set a text colour. On a light surface use the brand teal; on a
 * dark one, white. Inheriting an unrelated colour is how these end up nearly
 * invisible.
 */
...
export function FacebookIcon({ className = "w-5 h-5" }: { className?: string }) {
  return (
    <svg aria-hidden="true" className={className} fill="none" viewBox="0 0 24 24">
      {/* Disc and "f" in one path, even-odd knocking the letter out. */}
      <path
        fill="currentColor"
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20ZM14.64 12.58h-1.74V19h-2.66v-6.42H8.9v-2.26h1.34V8.86c0-1.11.53-2.86 2.86-2.86l2.1.01v2.34h-1.52c-.25 0-.78.13-.78.85v1.12h2.25l-.51 2.26Z"
      />
    </svg>
  );
}
