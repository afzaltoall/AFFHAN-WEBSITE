...
export function InquiryModal({ product, onClose }: InquiryModalProps) {
  ...
  // overlay, so we keep the user's scroll position — no jump to top — and just
  // restore the body overflow on close).
  useEffect(() => {
    if (!product) return;
  ...
  return (
    ...
    <AnimatePresence>
      {isLightboxOpen && images.length > 0 && (
        ...
        >
          <button onClick={() => setIsLightboxOpen(false)} className="absolute top-6 right-6 text-white/70 hover:text-white z-[130] p-2 transition-colors cursor-pointer hover:scale-110 active:scale-95">
            <X size={32} />
          </button>
          ...
          <TransformWrapper initialScale={1} minScale={0.5} maxScale={4} centerOnInit>
            {({ zoomIn, zoomOut, resetTransform }) => (
              <>
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-2 rounded-full z-[130]">
                  <button onClick={() => zoomOut()} className="p-2 text-white/70 hover:text-white hover:bg-white/20 rounded-full transition-colors cursor-pointer"><ZoomOut size={20} /></button>
                  <button onClick={() => resetTransform()} className="p-2 text-white/70 hover:text-white hover:bg-white/20 rounded-full transition-colors cursor-pointer"><Maximize size={18} /></button>
                  <button onClick={() => zoomIn()} className="p-2 text-white/70 hover:text-white hover:bg-white/20 rounded-full transition-colors cursor-pointer"><ZoomIn size={20} /></button>
                </div>
                ...
                {images.length > 1 && (
                  <>
                    <button 
                      ...
                      className="absolute left-6 top-1/2 -translate-y-1/2 text-white/70 hover:text-white p-3 rounded-full hover:bg-white/20 transition-all z-[130] cursor-pointer hover:scale-110 active:scale-95"
                    >
                      <ChevronLeft size={36} />
                    </button>
                    <button 
                      onClick={(e) => { e.stopPropagation(); setActiveImageIndex((prev) => (prev < images.length - 1 ? prev + 1 : 0)); resetTransform(); }} 
                      className="absolute right-6 top-1/2 -translate-y-1/2 text-white/70 hover:text-white p-3 rounded-full hover:bg-white/20 transition-all z-[130] cursor-pointer hover:scale-110 active:scale-95"
                    >
                      <ChevronRight size={36} />
                    </button>
                    
                    <div className="absolute bottom-20 left-1/2 -translate-x-1/2 flex items-center gap-2 z-[130]">
                      {images.map((img: string, idx: number) => (
                          ...
                          >
                            <Image src={getCdnUrl(img) as string} alt="Thumbnail" fill className="object-cover" />
                          </div>
                      ))}
                    </div>
                  </>
