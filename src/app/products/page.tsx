        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          ...
          {!debouncedQuery && (
            <div className="flex items-center gap-2.5">
              ...
              >
                {/* Gold laurel-trophy image (public/top-1.jpg). The source has a
                    thin gold border frame + white margin, so we crop it out with
                    clip-path and scale the laurel up to fill the icon cleanly. */}
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/top-1.jpg" alt="Top Ranking" width={22} height={22} className="h-[22px] w-[22px] object-contain object-center scale-[1.3] [clip-path:inset(9%)]" /> Top Ranking
              </Link>
            </div>
          )}
        </div>
            ...
            <div id="browse-categories" className="liquid-glass-card scroll-mt-28">
              ...
              {categoryPath.length > 0 && (
                <div className="flex items-center gap-2 px-4 sm:px-5 py-2.5 border-b border-slate-100">
                  ...
                  <button
                    onClick={() => goToCategory(null)}
                    className="shrink-0 inline-flex items-center gap-1 rounded-full border border-slate-200 px-2.5 py-1 text-[11px] font-semibold text-slate-500 hover:border-red-200 hover:text-red-500 transition-colors"
                  >
